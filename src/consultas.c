...
void consulta_dq(Lista listaForma,Lista listaQuadra, Lista listaHidrante, Lista listaSemaforo, Lista listaRadioBase, char aux[], char j[], double r, bool verificador, FILE *svg2, FILE* txt, FILE *txt_aux){
    ...
    anelDq(txt_aux, elemento, tipo_elemento, r);
    fprintf(txt,"\n\n");
}
...
void consulta_cv(Lista listaQuadra, double n, char cep[32], char face, double num, FILE* svg2, FILE* aux, Lista listaCasos){
    double xQ, yQ, wQ, hQ, x, y;
    char a[32];

    Texto text;

    Quadra quadra = get(listaQuadra, cep);
    Casos casos;
...
void consulta_soc(Lista listaQuadra, Lista listaPosto, int posto_atendimento, char cep[32], char face, double num, FILE* svg2, FILE* aux, FILE* txt){
    double xQ, yQ, wQ, hQ, x,y;  
    Texto text;
    Quadra quadra = get(listaQuadra, cep);
...
void consulta_ci(double x, double y, double r, FILE *aux, FILE *txt, Lista listaCasos, Lista listaPosto, Lista listaCasosDentro){
    ...
    if(categoriaDensidade < 0.1){
        ...
        if (flagPointInside(pontosPoligono, listaPosto) == false){
